...
export async function seedProductsIfEmpty() {
  ...
  if (count[0].count === 0) {
    const stmts = [
      {
        sql: `INSERT INTO products (id, name, sku, category, sale_price, cost_price, stock_qty, min_stock, track_stock, is_quick_add, is_active, created_at, updated_at) 
              VALUES ('1', 'شاحن أيفون أصلي', 'APL-CHG', 'accessory', 15000, 8000, 50, 5, 1, 1, 1, datetime('now'), datetime('now'))`
      },
      {
        sql: `INSERT INTO products (id, name, sku, category, sale_price, cost_price, stock_qty, min_stock, track_stock, is_quick_add, is_active, created_at, updated_at) 
              VALUES ('2', 'حماية شاشة أيفون 13', 'SCR-IP13', 'accessory', 5000, 2000, 100, 10, 1, 1, 1, datetime('now'), datetime('now'))`
      },
      {
        sql: `INSERT INTO products (id, name, sku, category, sale_price, cost_price, stock_qty, min_stock, track_stock, is_quick_add, is_active, created_at, updated_at) 
              VALUES ('3', 'خدمة فورمات وتنظيف', 'SRV-FMT', 'service_general', 10000, 0, 0, 0, 0, 0, 1, datetime('now'), datetime('now'))`
      },
      {
        sql: `INSERT INTO products (id, name, sku, category, sale_price, cost_price, stock_qty, min_stock, track_stock, is_quick_add, is_active, created_at, updated_at) 
              VALUES ('4', 'شريحة اتصال انترنت', 'SIM-NET', 'sim', 3000, 1500, 20, 5, 1, 0, 1, datetime('now'), datetime('now'))`
      }
    ];
